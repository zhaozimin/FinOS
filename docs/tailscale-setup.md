...
| 方案 | 安全 | 速度 | 复杂度 |
|---|---|---|---|
| 直接公网暴露 59418 | ❌ 极差（token 一旦泄露等同裸奔） | ★★★ | 简单 |
...
```
http://my-mac.tailxxxx.ts.net:59418/dashboard/?token=<your-token>
```

第一次访问会把 `?token=` 自动写入浏览器 LocalStorage，后续直接：
```
http://my-mac.tailxxxx.ts.net:59418/dashboard/
```
...
3. 访问：`http://my-mac:59418/dashboard/`
...
| 字段 | 填什么 |
|---|---|
| 服务器地址 | `http://my-mac.tailxxxx.ts.net:59418` |
...
3. 检查 Mac 防火墙是否拦了 59418：
   ```bash
   sudo pfctl -s rules | grep 59418
   ```
...
- ⚠️ **Tailscale node 共享**：如果你"share node"给别人，对方也能访问 59418。**别 share** 装了 FinOS 的 node。
- ✅ 可以用 Tailscale ACL 进一步限制：只允许某些 device 访问 Mac 的 59418 端口。
...
cloudflared tunnel --url http://127.0.0.1:59418
```
...
如果只在家用，连同一 Wi-Fi 就行：
```
http://192.168.x.x:59418/dashboard/?token=...
```
